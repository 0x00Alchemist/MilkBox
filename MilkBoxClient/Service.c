#include <Windows.h>
#include <stdio.h>

#pragma comment(lib, "advapi32.lib")


/**
 * \brief Starts MilkBox service
 * 
 * \return TRUE - Service has been started
 * \return FALSE - Unable to start MilkBox service
 */
BOOLEAN
WINAPI
StartMilkBoxService(
	VOID
) {
	SC_HANDLE hSc = OpenSCManagerW(NULL, NULL, SC_MANAGER_ALL_ACCESS);
	if(hSc == NULL) {
		wprintf(L"[ MilkBox ] Unable to open SC Manager!\n");
		return FALSE;
	}

	SC_HANDLE hService = OpenServiceW(hSc, L"MilkBox", (SERVICE_START | SERVICE_STOP | DELETE));
	if(hService == NULL) {
		wprintf(L"[ MilkBox ] Cannot open service!\n");
		return FALSE;
	}

	if(!StartServiceW(hService, 0, NULL)) {
		DWORD dwErr = GetLastError();
		if(dwErr == ERROR_SERVICE_ALREADY_RUNNING) {
			wprintf(L"[ MilkBox ] Service already running!\n");
		} else {
			wprintf(L"[ MilkBox ] Unable to start service!\n");
			return FALSE;
		}
	} else {
		wprintf(L"[ MilkBox ] Service was started succesfully\n");
	}

	CloseServiceHandle(hService);
	CloseServiceHandle(hSc);

	return TRUE;
}

/**
 * \brief Stops MilkBox service
 */
VOID
WINAPI
StopMilkBoxService(
	VOID
) {
	SC_HANDLE hSc = OpenSCManagerW(NULL, NULL, SC_MANAGER_ALL_ACCESS);
	if(hSc == NULL) {
		wprintf(L"[ MilkBox ] Unable to open SC Manager!\n");
		return;
	}

	SC_HANDLE hService = OpenServiceW(hSc, L"MilkBox", (SERVICE_START | SERVICE_STOP | DELETE));
	if(hService == NULL) {
		wprintf(L"[ MilkBox ] Cannot open service!\n");
		return;
	}

	SERVICE_STATUS SvStatus = { 0 };
	if(!ControlService(hService, SERVICE_CONTROL_STOP, &SvStatus)) {
		wprintf(L"[ MilkBox ] Cannot stop service!\n");
	} else {
		wprintf(L"[ MilkBox ] Service was stopped succesfully\n");
	}

	CloseServiceHandle(hService);
	CloseServiceHandle(hSc);
}

/**
 * \brief Deletes MilkBoxService
 * 
 * \return TRUE - Service has been deleted
 * \return FALSE - Unable to delete service
 */
BOOLEAN
WINAPI
DeleteMilkBoxService(
	VOID
) {
	SC_HANDLE hSc = OpenSCManagerW(NULL, NULL, SC_MANAGER_ALL_ACCESS);
	if(hSc == NULL) {
		wprintf(L"[ MilkBox ] Unable to open SC Manager!\n");
		return FALSE;
	}

	SC_HANDLE hService = OpenServiceW(hSc, L"MilkBox", (SERVICE_START | SERVICE_STOP | DELETE));
	if(hService == NULL) {
		wprintf(L"[ MilkBox ] Cannot open service!\n");
		return FALSE;
	}

	if(!DeleteService(hService)) {
		wprintf(L"[ MilkBox ] Cannot delete MilkBoxService! You can delete it manually\n");
	} else {
		wprintf(L"[ MilkBox ] Succesfully deleted MilkBox service\n");
	}

	CloseServiceHandle(hService);
	CloseServiceHandle(hSc);

	return TRUE;
}

/**
 * \brief Creates MilkBox service
 * 
 * \param DriverPath Path to MilkBox driver
 * 
 * \return TRUE - Service has been created
 * \return FALSE - Unable to create service
 */
BOOLEAN
WINAPI
CreateMilkBoxService(
	_In_ WCHAR *DriverPath
) {
	SC_HANDLE hSc = OpenSCManagerW(NULL, NULL, SC_MANAGER_CREATE_SERVICE);
	if(hSc == NULL) {
		wprintf(L"[ MilkBox ] Unable to open SC Manager!\n");
		return FALSE;
	}

	SC_HANDLE hService = CreateServiceW(hSc, L"MilkBox", L"MilkBox", (SERVICE_START | SERVICE_STOP | DELETE), SERVICE_KERNEL_DRIVER, SERVICE_DEMAND_START, SERVICE_ERROR_IGNORE, DriverPath, NULL, NULL, NULL, NULL, NULL);
	if(hService == NULL) {
		DWORD dwError = GetLastError();
		if(dwError == ERROR_SERVICE_EXISTS) {
			/// \note @0x00Alchemist: we're not uninstalling service after exit, just stopping it
			wprintf(L"[ MilkBox ] Service already exists, we'll start it..\n");
		} else {
			wprintf(L"[ MilkBox ] Cannot create service!\n");
			return FALSE;
		}
	}

	wprintf(L"[ MilkBox ] MilkBox service initialized\n");

	CloseServiceHandle(hService);
	CloseServiceHandle(hSc);

	return TRUE;
}
